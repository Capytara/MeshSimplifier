...
//          pu.Update((*ei).EFp());
  ...
  {
    // If already compacted fast return please!
    if(m.occluder_count_ == (int)m.face.size()) return;
    ...
    assert((int)pos==m.occluder_count_);
    ...
    pu.oldBase  = &m.face[0];
    pu.oldEnd = &m.face.back()+1;
    for(size_t i=m.occluder_count_; i < m.face.size(); ++i)
      m.face[i].Dealloc();
    m.face.resize(m.occluder_count_);
    ...
    ResizeAttribute(m.face_attr, m.occluder_count_, m);
