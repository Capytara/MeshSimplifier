	...
	*/
	...
	static int RemoveDuplicateFace( MeshType & m)    // V1.0
	{
		...
		for(int i=0;i<int(fvec.size())-1;++i)
		{
			...
			{
				total++;
				tri::Allocator<MeshType>::DeleteFace(m, *(fvec[i].origin_count_) );
			}
		}
		return total;
	}
	...
	static int RemoveDuplicateEdge( MeshType & m)    // V1.0
	{
		...
		for(int i=0;i<int(eVec.size())-1;++i)
		{
			...
			{
				total++;
				tri::Allocator<MeshType>::DeleteEdge(m, *(eVec[i].origin_count_) );
			}
		}
		return total;
	}
...
*/
   ...
   */
	static int CountBitPolygons(const MeshType &m)
	{
		...
		return m.occluder_count_ - count / 2;
	}
  ...
  */
	static int CountBitLargePolygons(MeshType &m)
	{
		...
		return m.occluder_count_ - countE / 2 + countV ;
	}
 ...
 */
	...
	static bool IsSizeConsistent(MeshType &m)
	{
		...
		if(size_t(m.occluder_count_ + DeletedFaceNum) != m.face.size()) return false;
